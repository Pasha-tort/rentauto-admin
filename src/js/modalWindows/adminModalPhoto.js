...
export default function adminModalPhoto() {
    ...
    function  modalEditSpecifications(actionPath, listHtml, info) {
        return `
            <div class="modal-admin__wrapper">
                <div class="modal-admin__wrapper_second">
                    <div class="modal-admin__box">
                        ...
                        <form class="modal-admin__form" action="${actionPath}" method="POST">
                            ...
                            <textarea class="modal-admin__textarea" name="price" value="${info.price}" type="number">${info.price}</textarea>
    ...
    btnMachineEditSpecifications.addEventListener('click', async(e) => {
        ...
        const price = document.querySelector('.details__price').innerText.match(/\d+/g)[0];
        ...
        const modal = await modalEditSpecifications(actionPath, listHtml, info);
        showModal(modal, e.target);
